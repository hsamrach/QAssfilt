            ...
            case "$TOOL" in
				...
				spades.py)
					...
					if ! spades.py --version &>/dev/null; then
						echo "❌ SPAdes installation failed!"
						exit 1
					fi
					echo "[INFO] SPAdes installation complete: $(spades.py --version)"
					;;
				...
				checkm2)
					...
					if checkm2 --version >/dev/null 2>&1; then
						echo "✅ CheckM2 installation complete: checkm2 version $(checkm2 --version)."
					else
